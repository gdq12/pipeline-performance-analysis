...
create or replace external table `${PROJECT_ID}`.`nytaxi_raw`.`mapping_data`
options (
format = 'CSV',
uris = ['gs://helper-data/bigquery_public_data_taxi_zone_geom.csv']
)
;
...
options (
format = 'CSV',
uris = ['gs://helper-data/hvlv_base_numbers.csv'],
skip_leading_rows = 1
)
;
...
options (
format = 'CSV',
uris = ['gs://helper-data/taxi_zone_lookup.csv']
)
;
