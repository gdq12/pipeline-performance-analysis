  ...
  create or replace view `pipeline-analysis-455005`.`nytaxi_clean`.`yellow__3_data_type_cast`
  OPTIONS()
  as with trps as 
(select 
...
from `pipeline-analysis-455005`.`nytaxi_clean`.`yellow__2b_2009_2010_location_id_update`
union all 
select 
    case safe_cast(vendor_id as string)
        when '1' then 1
    ...
    ,cast(creation_dt as timestamp) creation_dt
    ,cast(clone_dt as timestamp) clone_dt
from `pipeline-analysis-455005`.`nytaxi_clean`.`yellow__2_post_2010_tbl_collation`
)
select 
  vendor_id,
  pickup_datetime,
