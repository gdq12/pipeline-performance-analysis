    ...
    create or replace table `pipeline-analysis-455005`.`nytaxi_mart2`.`mart2_dm_monthly_stats`
  ...
  (
    ...
    partition by timestamp_trunc(pickup_month, month)
    

    OPTIONS()
    as (
      
    select pickup_borough, pickup_zone, pickup_service_zone, dropoff_borough, dropoff_zone, dropoff_service_zone, pickup_month, pickup_year, pickup_rush_hour_status, pickup_weekday_name, applied_rate, payment_type, trip_type, hvfs_license_name, avg_trip_distance, avg_trip_duration_min, fare_amount, tip_amount, total_amount, total_fees, passenger_count, num_public_holidays, num_trips, transformation_dt
    from (
