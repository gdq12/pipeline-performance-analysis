  ...
  create or replace view `pipeline-analysis-455005`.`nytaxi_stage`.`stg_yellow__2_filter_out_faulty`
  OPTIONS()
  as select 
  ...
  tbl.creation_dt,
  tbl.clone_dt
from `pipeline-analysis-455005`.`nytaxi_clean`.`yellow__4_adds_columns` tbl
left join `pipeline-analysis-455005`.`nytaxi_stage`.`stg_yellow__1b_id_faulty_trips` ft1 on tbl.data_source = ft1.data_source 
                                                    and tbl.trip_id = ft1.trip_id
left join `pipeline-analysis-455005`.`nytaxi_stage`.`stg_yellow__1a_id_duplicate_records` ft2 on tbl.pickup_datetime = ft2.pickup_datetime
