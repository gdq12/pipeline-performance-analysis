select 
    data_source, 
    trip_id
from {{ ref('fhv__4_adds_columns') }} fhv 
left join {{ source('mapping.map', 'hvlv_base_numbers') }} mp on fhv.dispatching_base_number = mp.license_base_number
left join {{ source('mapping.map', 'hvlv_base_numbers') }} mp2 on fhv.affiliated_base_number = mp2.license_base_number
where (
